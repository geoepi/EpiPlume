...
extract_pipeline_receptor_result <- function(parsed_result, facilities, cfg) {
  ...
  tryCatch({ extraction <- extract_facility_receptors_from_plume(parsed_result$parsed_plume, facilities, cfg, cfg$pipeline$write_intermediate_outputs); list(run_id = parsed_result$run_id, status = "receptor_extracted", extraction = extraction, error_message = NA_character_) }, error = function(e) list(run_id = parsed_result$run_id, status = "failed", extraction = NULL, error_message = conditionMessage(e)))
}
