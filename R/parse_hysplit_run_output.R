...
parse_hysplit_run_output <- function(run_metadata, cfg, write_outputs = FALSE, overwrite = FALSE) {
  ...
  if (isTRUE(write_outputs)) parsed$parsing_metadata <- write_hysplit_parsed_outputs(parsed, run_metadata$run_directory, overwrite)
  parsed
}
